...
gchar* cb_open (GtkWidget *p_widget, gpointer user_data)
{
            GError *gerror;
  GtkWidget *p_dialog = NULL;
    GtkWidget *foto = NULL;
         const static gchar *file_name = NULL;
  ...
  if (gtk_dialog_run (GTK_DIALOG (p_dialog)) == GTK_RESPONSE_ACCEPT)
  {
      ...
      pixbuf2 = gdk_pixbuf_new_from_file(file_name, gerror);
  ...
  /* Parametre inutilise */
  (void)p_widget;
    printf("Filepath is : %s",file_name);
    return file_name;
}
...
void cb_save (GtkWidget *p_widget, gpointer user_data)
{
    GtkWidget *p_dialog = NULL;
    GtkWidget *foto = NULL;
    gchar *file_name = cb_open(p_widget,user_data);
    //p_dialog = gtk_dialog_new();
  /*  p_dialog = gtk_file_chooser_dialog_new ("Ouvrir un fichier", NULL,
                                            GTK_FILE_CHOOSER_ACTION_OPEN,
                                            GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
                                            GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT,
   ...
   // if (gtk_dialog_run (GTK_DIALOG (p_dialog)) == GTK_RESPONSE_ACCEPT)
    //{
        ...
        GdkPixbuf *rpb, *ipb;
        GdkPixbuf *pixbuf2 = gdk_pixbuf_new_from_file("/tmp/input.png", gerror);
        originalpb = gdk_pixbuf_new_from_file_at_size (file_name,24,24,&gerror);
        printf("JE SUIS LA \n");
